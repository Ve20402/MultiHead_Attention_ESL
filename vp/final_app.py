...
class AttentionWhisperBlock(WhisperAttention):
    ...
    def forward(self, hidden_states, key_value_states=None, past_key_value=None,
        ...
        print(f"[PYTHON-INPUT] Q[0,0,0] after scale: {q_list[0,0,0]}")
        print(f"[PYTHON-INPUT] K[0,0,0]: {k_list[0,0,0]}")
        print(f"[PYTHON-INPUT] V[0,0,0]: {v_list[0,0,0]}")
        if not os.path.exists("../data/matrice"):
            os.makedirs("../data/matrice")
        ...
        np.savetxt("../data/matrice/multihead_ulaz_Q.txt", q_list[0])
        np.savetxt("../data/matrice/multihead_ulaz_K.txt", k_list[0])
        np.savetxt("../data/matrice/multihead_ulaz_V.txt", v_list[0])

        #Čuvamo W_out i b_out
        #Moramo transponovati W jer PyTorch cuva kao (Out, In), a mi mnozimo (In, Out)
        np.savetxt("../data/matrice/multihead_W_out.txt", self.out_proj.weight.detach().cpu().numpy().T)
        np.savetxt("../data/matrice/multihead_b_out.txt", self.out_proj.bias.detach().cpu().numpy())
        ...
        print("Cuvanje output-a iz Pythona...")
        final_golden_np = attn_output.detach().cpu().numpy()
        np.savetxt("../data/matrice/output_main_app.txt", final_golden_np[0], fmt='%.8f')
