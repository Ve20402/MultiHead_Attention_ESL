...
void MatrixMultiplier::mem_read_block(uint64_t addr, float* data_ptr, uint32_t num_elements) {
    ...
    if (dmi_ptr_valid) {
        ...
        unsigned int cycles = num_elements; // pretpostavimo 1 element po ciklusu (32-bitna magistrala)
...
void MatrixMultiplier::multiply() {
    while (true) {
        ...
        for (uint32_t i = 0; i < rows; i++) {
            ...
            qk.inc(sc_time((cols / 8) * w_rows * 10, SC_NS));   // unroll = 8, pa zbog toga delimo sa 8, ali moze i bez toga
            qk.set_and_sync(qk.get_local_time());
