...
void RamTLM::b_transport(int id, tlm_generic_payload& trans, sc_time& delay) {
    ...
    if (addr + len > size) {    // proveri da li je adresa van opsega
    ...
    if (cmd == TLM_READ_COMMAND) {
        ...
        delay += sc_time(cycles * 10, SC_NS);
    } else if (cmd == TLM_WRITE_COMMAND) {
        memcpy(&mem[addr], ptr, len);
        unsigned int cycles = ceil((float)len / 4);   //podatke na koje pokazuje ptr upisi u memoriju na adresu addr. podaci su duzine len
        delay += sc_time(cycles * 10, SC_NS);
    }
